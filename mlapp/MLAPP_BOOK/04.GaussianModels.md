...
所有向量都假设为列向量(column vector),除非特别说明是行向量.通过堆叠(stack)D个标量(scalar)得到的类向量记作$[x_1,...,x_D]$.与之类似,如果写,那么等号左侧就是一个高列向量(tall column vector),意思就是沿行堆叠,一般写作 ,不过这样很丑哈.如果写,等号左边的是矩阵,意思就是沿列堆叠,建立一个矩阵.
...
指数函数内部的是一个数据向量**x**和均值向量**** 之间的马氏距离(马哈拉诺比斯距离,Mahalanobis distance).对$\Sigma$进行特征分解(eigendecomposition)有助于更好理解这个量.,其中的U是标准正交矩阵(orthonormal matrix),满足,而是特征值组成的对角矩阵.经过特征分解就得到了:
